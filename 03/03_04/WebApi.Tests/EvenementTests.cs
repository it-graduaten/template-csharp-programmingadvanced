...
namespace WebApi.Tests
{
    public class EvenementEndpointsTests
    {
                ...
                .WithWebHostBuilder(builder =>
                {
                    builder.ConfigureServices(services =>
                    {
                        var descriptor = services.SingleOrDefault(
                            d => d.ServiceType == typeof(IEvenementRepository));
                        if (descriptor != null)
                        {
                            services.Remove(descriptor);
                        }
                        services.AddSingleton<IEvenementRepository, InMemoryEvenementRepository>();
                    });
                });
        ...
        [Test]
        public async Task GetEvenementenByLocatie_Antwerpen_GeeftEenEventTerug()
        {
            // Act
            HttpResponseMessage response = await _client.GetAsync("/evenementen/locatie/Antwerpen");
            var evenementen = await response.Content.ReadFromJsonAsync<List<Evenement>>();

            // Assert
            Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.OK));
            Assert.That(evenementen, Is.Not.Null);
            Assert.That(evenementen!.Count, Is.EqualTo(1));
            Assert.That(evenementen[0].Naam, Is.EqualTo("Summer Music Festival"));
        }

        [Test]
        public async Task GetEvenementenByLocatie_Brugge_GeeftEenEventTerug()
        {
            // Act
            HttpResponseMessage response = await _client.GetAsync("/evenementen/locatie/Brugge");
            var evenementen = await response.Content.ReadFromJsonAsync<List<Evenement>>();
            ...
            Assert.That(evenementen, Is.Not.Null);
            Assert.That(evenementen!.Count, Is.EqualTo(1));
            Assert.That(evenementen[0].Naam, Is.EqualTo("Culinaire Dagen"));
        }

        [Test]
        public async Task GetEvenementenByLocatie_OngevoeligVoorHoofdletters()
        {
            // Act - lowercase
            HttpResponseMessage responseLower = await _client.GetAsync("/evenementen/locatie/antwerpen");
            var evenementenLower = await responseLower.Content.ReadFromJsonAsync<List<Evenement>>();

            // Act - uppercase
            HttpResponseMessage responseUpper = await _client.GetAsync("/evenementen/locatie/ANTWERPEN");
            var evenementenUpper = await responseUpper.Content.ReadFromJsonAsync<List<Evenement>>();

            // Assert - both should return the same results
            Assert.That(evenementenLower, Is.Not.Null);
            Assert.That(evenementenUpper, Is.Not.Null);
            Assert.That(evenementenLower!.Count, Is.EqualTo(evenementenUpper.Count));
        }

        [Test]
        public async Task GetEvenementenByLocatie_OnbekendeLocatie_GeeftLegeLijstTerugMetStatusOk()
        {
            // Act
            HttpResponseMessage response = await _client.GetAsync("/evenementen/locatie/Kortrijk");
            var evenementen = await response.Content.ReadFromJsonAsync<List<Evenement>>();

            // Assert
            Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.OK));
            Assert.That(evenementen, Is.Not.Null);
            Assert.That(evenementen!.Count, Is.EqualTo(0));
        }
